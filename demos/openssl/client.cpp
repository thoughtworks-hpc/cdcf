...
caf::actor connect_tcp_worker(caf::actor_system &system, const std::string &host) {
...
caf::actor connect_ssl_worker(caf::actor_system &system, const std::string &host) {
    auto result_ssl = caf::openssl::remote_actor(system, host, 50603);
    if (!result_ssl) {
        std::cout << "ssl connect failed, reason: " << system.render(result_ssl.error()) << std::endl;
        exit(1);
    }
    std::cout << "address: " << caf::to_string(result_ssl->address()) << std::endl;
    return *result_ssl;
}
...
// Run configuration
// program args: --openssl.cafile=ca-noenc.crt --openssl.certificate=node2.cert.pem --openssl.key=node2.key.pem
// working directory: /path/to/docker/ssl