...
void caf_main(actor_system &system, const actor_system_config& cfg) {
    ...
    std::cout << "spong id: " << spong.id() << ", address: " << caf::to_string(spong.address()) << std::endl;
    expected_port = caf::io::publish(spong, 50602);
    if (!expected_port) {
        std::cout << "publish spong failed. reason: " << system.render(expected_port.error()) << std::endl;
        exit(1);
    }
    auto spong_ssl = system.spawn(make_pong_ssl_behavior);
    std::cout << "spong_ssl id: " << spong_ssl.id() << ", address: " << caf::to_string(spong_ssl.address()) << std::endl;
...
// Run configuration
// program args: --openssl.cafile=ca-noenc.crt --openssl.certificate=node1.cert.pem --openssl.key=node1.key.pem
