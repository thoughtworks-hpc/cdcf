...
void membership::Membership::PullFromSeedMember() {
    ...
    CDCF_LOGGER_DEBUG("---------------------------------------");
    CDCF_LOGGER_DEBUG("PullFromSeedMember");
    CDCF_LOGGER_DEBUG("---------------------------------------");
    if (transport_) {
        ...
        transport_->Pull(random_address, pull_request_message.data(),
                         ...
                         [random_address,
                             ...
                             CDCF_LOGGER_INFO("Pull from SEED node");
                             PullFromSeedMember();
                         });
    }
}
...
void membership::Membership::HandleDidPull(
    ...
    CDCF_LOGGER_DEBUG("-----------------------------");
    CDCF_LOGGER_DEBUG("HandleDidPull");
    CDCF_LOGGER_DEBUG("-----------------------------");
    if (result.first == gossip::ErrorCode::kOK) {
        ...
        if (message.IsSuccess()) {
            for (const auto &member : message.GetMembers()) {
                CDCF_LOGGER_ERROR("name: {}", member.GetNodeName());
                CDCF_LOGGER_ERROR("hostname: {}", member.GetHostName());
                CDCF_LOGGER_ERROR("uid: {}", member.GetUid());
                CDCF_LOGGER_ERROR("role: {}", member.GetRole());
                CDCF_LOGGER_ERROR("ip: {}", member.GetIpAddress());
...
std::vector<uint8_t> membership::Membership::HandlePull(
    ...
    if (request.IsFullStateType()) {
        ...
        for (const auto &member : members_) {
            CDCF_LOGGER_DEBUG("Received full state pull request member: ");
            CDCF_LOGGER_DEBUG("name: {}", member.first.GetNodeName());
            CDCF_LOGGER_DEBUG("hostname: {}", member.first.GetHostName());
            CDCF_LOGGER_DEBUG("ip: {}", member.first.GetIpAddress());
            CDCF_LOGGER_DEBUG("role: {}", member.first.GetRole());
            CDCF_LOGGER_DEBUG("uid: {}", member.first.GetUid());
        ...
        CDCF_LOGGER_DEBUG("members.size() : {}", members.size());
        ...
        CDCF_LOGGER_DEBUG("-----------------------------");
        CDCF_LOGGER_DEBUG("message_serialized: {}", message_serialized);
        CDCF_LOGGER_DEBUG("-----------------------------");
